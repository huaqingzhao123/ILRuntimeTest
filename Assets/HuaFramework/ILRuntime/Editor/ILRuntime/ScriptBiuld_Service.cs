...
public class ScriptBiuld_Service
{
        ...
        bool isdebug = false)
    {
        ...
        try
        {
            //剔除热更脚本,编译Assembly-CSharp.dll,baseCs
    ...
    static List<string> FindDLLByCSPROJ(string projName, ref List<string> dllList)
    {
        ...
        var projpath = UnityTools.ProjectRoot + "/" + projName;
        ...
        foreach (XmlNode childNode in ProjectNode.ChildNodes)
        {
            ...
            else if (childNode.Name == "PropertyGroup")
            {
                foreach (XmlNode item in childNode.ChildNodes)
                {
                    if (item.Name == "DefineConstants")  //宏定义
                    {
                        define = item.InnerText;
                        //Debug.Log(projName + " 宏定义：" + define);
                    }
                }
            }
        }
        ...
        foreach (var csproj in csprojList)
        {
            ...
            //添加扫描到的dll(递归添加其他工程引用的dll)
            FindDLLByCSPROJ(csproj, ref dllList);
            //将工程dll加入进来
            var gendll = UnityTools.Library + "/ScriptAssemblies/" + csproj.Replace(".csproj", ".dll");
